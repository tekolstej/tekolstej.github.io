...
if not sheet.get_all_values():
    sheet.append_row([
        ...
        "Float Sensor 1", "Float Sensor 2", "Water Level"
    ])
...
def determine_water_level(float1, float2):
    if float1 == 0 and float2 == 0:  # Both sensors triggered
        return "Low"
    elif float1 == 0 and float2 == 1:  # Only bottom sensor triggered
        return "Half"
...
try:
    while True:
        ...
        float1_status = lgpio.gpio_read(h, FLOAT1_PIN)
        float2_status = lgpio.gpio_read(h, FLOAT2_PIN)
        ...
        print(f"{timestamp} - Temp: {temperature:.2f} °C, Pressure: {pressure:.2f} hPa, "
              ...
              f"Float 1: {'Triggered' if float1_status == 0 else 'Not Triggered'}, "
        ...
        with open(log_file, "a", newline="") as csvfile:
            ...
            writer.writerow([timestamp, temperature, pressure, humidity, ph_voltage, pH_value,
                             tds_voltage, tds_value, 
                             "Triggered" if float1_status == 0 else "Not Triggered", 
                             "Triggered" if float2_status == 0 else "Not Triggered",
                             water_level])
