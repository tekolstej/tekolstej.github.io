...
def determine_water_level(float1_status, float2_status):
    if float1_status == 0 and float2_status == 0:  # Both sensors triggered
        return "High"
    elif float1_status == 1 and float2_status == 0:  # Only bottom sensor triggered
        return "Over Half"
    elif float1_status == 1 and float2_status == 1:  # Both sensors not triggered
        return "Low"
    elif float1_status == 0 and float2_status == 1:  # Only top sensor triggered
...
def main():
    try:
        ...
        # Step 2: Start 10-second timer to maintain water level
        print("Starting 10-second timer to maintain water level.")
        ...
        print("10 seconds elapsed. Starting output pump to drain bucket.")
        while True:
            float1_status, float2_status = read_float_sensors()
            water_level = determine_water_level(float1_status, float2_status)
        ...
        exit(0)  # Ensure the program stops completely
